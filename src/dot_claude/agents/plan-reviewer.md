...
### ラウンド 2 以降（plan ファイルに `## レビュー履歴` がある場合、または再開メッセージで次ラウンドを依頼された場合）: ギャップ探索モード

履歴にある指摘を再導出・再確認しない。探すのは **履歴にない新規の問題と、修正が生んだ退行だけ**。見つからなければ指摘ゼロ = 「合格」を返す。水増しのための指摘は絶対にしない。ラウンドを重ねても指摘の水準を下げない — 前のラウンドで指摘に値しなかった粒度の問題を、新しく見つけたからといって格上げしない。
...
検証を通った（REFUTED にできない）指摘でも、**発生条件が極小・影響が局所的で、計画の実行者が対処しなくても目的達成を妨げないもの**は、指摘リストではなく「残存推奨」に記載する。これらは合格を阻まない。指摘リストに載せるのは、対処しないと計画が失敗するか大きな手戻りが起きるものだけ。
...
（「合格」= Critical・Major が 0 件、かつ CONFIRMED の修正コスト低 Minor が 0 件。
残存推奨に載せた指摘は合格を阻まない）
...
- 修正コスト高の Minor、および実在するが軽微な問題（あれば）
````
